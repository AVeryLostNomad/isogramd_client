	...
	public class PagerIndicatorTabs : Grid
	{
		...
		void CreateTabs()
		{
			...
			foreach (var item in ItemsSource)
			{
				...
				{
					case Device.iOS:
						tab.Children.Add(new Image { Source = ImageSource.FromResource(((HomeViewModel) item).ImageSource), HeightRequest = 20 });
						tab.Children.Add(new Label { Text = ((HomeViewModel) item).TabText, FontSize = 11 });
						break;

					case Device.Android:
						tab.Children.Add(new Image { Source = ImageSource.FromResource(((HomeViewModel)item).ImageSource), HeightRequest = 25 });
						tab.Children.Add(new Label { Text = ((HomeViewModel)item).TabText, FontSize = 11 });
						break;
				}
